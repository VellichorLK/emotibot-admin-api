...
func finishJSONReport(r *Report, writeToFile bool, f *os.File) ([]byte, string, error) {
	...
	for _, v := range r.Records {
		duration := float64(int64((float64(v.Duration)/1000)*100)) / 100
		...
		t = time.Unix(int64(v.ProcessET), 0)
...
func finishCSVReport(r *Report, writeToFile bool, f *os.File) ([]byte, string, error) {
	...
	ctx += "#," + NFILENAME + "," + NDURATION + "," + NUPT + "," + NFINT + "\n"
	...
	ctx += NSUMD + "," + strconv.FormatFloat(r.CumulativeDuration, 'f', 2, 64) + "\n"
...
func makeCSVRow(r *Report, idx int) (string, error) {
	...
	duration := float64(int64((float64(r.Records[idx].Duration)/1000)*100)) / 100
	...
	//row += t.Format(TimeFormat) + "\n"
	//r.Records[idx].ProcessET = r.Records[idx].ProcessET
