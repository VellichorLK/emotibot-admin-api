...
func SystemSettingHandler(w http.ResponseWriter, r *http.Request, c *Configuration, d *DaoWrapper) {
	...
	var err error
	s.ChartInfo = &ChartProp{}
	s.ChartInfo.XMax, err = strconv.ParseFloat(r.FormValue("x_max"), 64)
	if err != nil {
		s.ChartInfo.XMax = 36
	}
	s.ChartInfo.XMin, err = strconv.ParseFloat(r.FormValue("x_min"), 64)
	if err != nil {
		s.ChartInfo.XMin = 0
	}
	s.ChartInfo.YMax, err = strconv.ParseFloat(r.FormValue("y_max"), 64)
	if err != nil {
	...
	LogInfo.Printf("xMax: [%.5f], xMin: [%.5f], yMax: [%.5f], yMin: [%.5f]\n", s.ChartInfo.XMax, s.ChartInfo.XMin, s.ChartInfo.YMax, s.ChartInfo.YMin)
