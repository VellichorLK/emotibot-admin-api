...
func newResponseArg(text string) *responseArg {
	return &responseArg{
		Text0:      text,
		Text1:      text,
		Text1Old:   text,
...
func (r *responseClient) newRequest(sentence string) (*http.Request, error) {

	arg := newResponseArg(sentence)
...
func (r *responseClient) Post(sentence string) (*rresponse, error) {
	req, err := r.newRequest(sentence)
	if err != nil {
