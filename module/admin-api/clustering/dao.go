...
// func GetQuestionIDByContent(appid string, content []interface{}) (map[string]int, error) {
// 	db := util.GetMainDB()
// 	if db == nil {
...
// 	tableName := fmt.Sprintf(QuestionTableFormat, appid)
// 	querySQL := fmt.Sprintf(`SELECT %s,%s FROM %s WHERE %s in (?, %s)`,
// 		NQuestionID, NContent, tableName, NContent, strings.Repeat(",?", len(content)-1))
